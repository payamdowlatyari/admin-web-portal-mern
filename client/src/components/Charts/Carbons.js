...
const CustomizedDot = (props) => { //This is used for creating the smiley faces for the carbon deferral graph
  const {
    cx, cy, value,
  } = props;

  if (value > 1500) { //Graph is taken from 'recharts' api
    return (
...
export default class Carbons extends PureComponent {
  ...
  componentDidMount() { //axios api call for retrieving the data from the 'emissions' collection in the db
    axios.get("https://cpmqtt1.calit2.uci.edu/api.php?collection=emissions")
      .then(res => {
        this.setState({ data: res.data.result }); 
      })
  }
  // These graphs are associated with "carbon deferral" and "carbon emissions"
  render() {
    return (
      <Container>
